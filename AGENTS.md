...
- 화면 투영은 프리셋 5종(2:1 다이메트릭 기본 · 트루 아이소 · 밀리터리 1:1 · 3/4 톱다운 · 레퍼런스 3/4 직교)을
  플레이 중 **F9 / F8** 로 전환해 비교할 수 있다. 사용법과 구현은
  `docs/unity-port/projection-presets.md`. 투영값은 `IsometricProjection` 한 곳에만 둔다.
