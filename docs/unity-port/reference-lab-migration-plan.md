...
#### 본선 A — R2 미리보기 적용 (2026-09-10, `RunBootstrap._r2Look`)

레이어 12종 없이도 옮길 수 있는 항목만 본선에 얹었다. `_r2Look` 스위치로 같은 씬에서 R1↔R2 를 비교한다.

| 항목 | 본선 적용 |
|---|---|
| 어둠 오버레이 | 퍼플 어둠·기억 색 + **supersample 4** (`BindDarkness()` — 층 전환 RebindWorld 에서도 같은 값) |
| **보이는 벽 칸 가시도 0.5** | `DarknessOverlay.SetSolidVisibility` 신설. LOS 가 벽 타일까지 표시해도 그 칸은 절반만 드러난다 — 본선 벽 타일은 윗면·정면이 한 장이라 레이어로 못 나누므로, "윗면은 빛을 못 받는다"를 이 값으로 흉내낸다. 랩은 1(끔) |
| 전역광 색 | 랩 AmbientHue (0.62,0.50,0.76). 세기는 본선 0.16 유지(랩 0.14 와 근접) |
| 손전등 그림자 | 0.9/0.35 → Scout 규칙 0.70/0.55 |
| 램프 그림자 | Worklamp 규칙 0.45/0.75. 색·반경·세기는 본선 정의(넓은 주황) 유지 — **결정 ① 은 "본선은 주황, 랩 레퍼런스 램프는 마젠타(다른 소품)"로 잠정** |

**옮길 수 없었던 것과 이유.** 전역광 2분할·벽 그림자 수신 레이어 제한·지면/윗면 광원 분리는 전부 <b>소팅 레이어</b>가 전제다.
본선은 바닥·벽·적·드롭·FX 가 모두 `Default` 한 레이어(order 0/10/25~60)에 있고, Unity 소팅 레이어에서 `Default` 는 항상
...
#### 본선 B 9·10단계 (2026-09-10) ✅

| 단계 | 적용 | 실측 |
|---|---|---|
| **9 윤곽 캐스터** | 환경 렌더러 모드에서 `WallShadowBuilder`(청크 사각) 대신 `ShadowGeometryBuilder` 를 `Environment Root` 에 세우고 `Bind(_env.IsWallCell, cols, rows)`. 셀 변경 → `MarkDirty()`(표면 dirty 와 같은 프레임). 발밑 캐스터(`WallShadowBuilder.AttachActorCaster`, 정적)는 그대로 | 캐스터 **366 → 20**(윤곽 16 + 발밑 4), 전부 바닥 4레이어 수신. 구 `WallShadows` 루트 없음 |
| **10 광원 소켓** | `LightSocketRenderer` 를 조명 루트에. 손전등·크루 손전등 → Scout 소켓, 램프 → Worklamp 소켓(설치 높이 0 = 지면). 후광·플레어·보스 광원은 소켓 밖(세기를 코드가 애니메이션) | Scout 4(플레이어+크루 3) 그림자 ON 0.70/0.55 · Worklamp 6 깜빡임 1.04~1.16 · 그림자 OFF |
...
#### 본선 B 12단계 — 회귀 (2026-09-10) ✅ (1차)

| 항목 | 결과 |
|---|---|
| 층 전환 | `DoDescend()` 1→2: 예외 0, 다음 프레임 `Environment Grid` 1개(Bind 가 자식 파괴·재생성, Destroy 는 프레임 끝), 캐스터 41→13·소켓 19→7 로 정리 — 누수 없음. 지층 2 키트가 비어 있어 `KitForDepth` 가 지층 1 로 폴백 |
| 채굴 동시 갱신 | `ForceClear` ×3: 표면 dirty → 캐스터 366→368(구 빌더 시점) / 균열 단계 / 드롭섀도 프레임 끝 1회 / LOS 버전 — 같은 프레임 경로 확인(7단계 후속 프로브) |
| 성능 예산 | 벽 캐스터 366 → **13~20**(윤곽), 광원 11, 그림자 예산 4(High) 집행, 어둠 텍스처 320×288 4×, 드롭섀도 Resync 프레임당 ≤1 |
| **EditMode 테스트** | **400/400 통과.** 신설 `LosServiceTests`(벽 표시 후 차단 · 범위 밖 고체 · 탐색 영구 · 기억 바닥 30% · 버전 캐시 · 크루/보스 시야원) 6개. `SurfaceTopologyTests.벽_발밑_바닥에_접촉AO` 를 4방향 계약으로 갱신(격자 가장자리는 고체 → AO) |

미완: 폭발·보스 지형 변경(대량 파괴)은 자동 프로브로 못 돌렸다 — 플레이로 확인 필요. PlayMode 테스트 없음.

**B 완료.** 남은 것은 아트 도착 시 `RunStrata` 재실행, 그리고 미뤄둔 손전등 점멸·보스 벽 아트 교체.

#### 어둠 경계 — 4× 초해상 폐기, 원본 fogProg 합성 이식 (2026-09-10 저녁)

피드백: "암흑이 네모네모로 딱딱하고, 시야와 어둠이 너무 빨리 스왑돼 맵이 정신없다." 6단계 후속 1 에서 "경계가 뭉개진다"를 고치려고
supersample 4 로 죄었던 것이 정확히 반대로 갔다. 원본 HTML `v7.9.2` 의 `fogProg` 가 이미 답을 갖고 있었다:

| 원본 | 이식(`Darkness.shader`) |
|---|---|
| 1.35칸 간격 **9탭 필터**(0.24 / 0.12×4 / 0.07×4) → 약 1.5칸 반그림자 | 그대로. 오프셋은 월드 셀 단위(`_WorldSize`)라 텍스처 해상도와 무관 |
| 경계에만 **월드 고정 노이즈** `edge=4v(1−v)`, ±0.08 | 그대로 — 완벽한 원·타일 윤곽을 깨고, 카메라 이동 중 크롤링 없음 |
| `visible=smoothstep(0.035, 0.88)` · `memory=smoothstep(0.004,0.24)×0.22` | 그대로. `_EdgeSoftness` = 상한(0.88) |
| `known=max(visible, memory×1.45)` → `unseen=1−smoothstep(0.015,0.46,known)` | 어둠 알파 |
| 이징 rise 0.16s / fall 0.38s | 이미 같음(`DarknessOverlay`) |

supersample 은 랩·본선 모두 **1** 로 복귀(80×72). 프리셋 `losEdgeSoftness` 기본 0.35 → **0.88**. 원본의 Bayer 디더 포스터라이즈는 넣지 않았다(URP 조명이 연속값).
