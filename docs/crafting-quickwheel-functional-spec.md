...
모든 아이콘은 256×256 RGBA, 중앙 정렬, 투명 배경이다. 휠에서는 92×92px, 상세 패널에서는 68×68px로 표시한다. 월드 배치 미리보기는 고정 픽셀이 아니라 현재 줌의 1타일 규격에 맞추며, 방벽은 2×1타일 프레임 안에 표시한다.
...
- 설치 단계에 진입한 순간의 `G.Z`를 고정한다. 크루 간격에 따른 동적 줌과 `[`/`]` 수동 줌은 설치가 끝날 때까지 적용하지 않는다.
- 마우스의 월드 좌표를 `toCell()`로 변환한 뒤 `cxw()`/`cyw()`의 타일 중심에 스냅한다. 미리보기와 최종 설치가 동일한 타일 좌표를 사용한다.
- 미리보기 프레임은 현재 줌에서 정확히 한 타일 크기로 표시한다. 방벽은 방향에 따라 2×1 또는 1×2 타일 규격을 표시한다.
- 2칸 이상을 차지하는 설치물은 좌상단 앵커 타일부터 점유 칸 전체를 검사하며, 하나라도 벽·맵 밖이면 설치 불가로 판정한다.
- 커서가 같은 타일 안에서 움직이는 동안 미리보기는 움직이지 않으며, 타일 경계를 넘을 때만 다음 타일 중심으로 이동한다.
...
- 비용: PULP 6.
- 설치 거리: 최대 `CELL * 2.3`.
- 플레이어 조준 방향에 수직인 방향을 기준으로 가장 가까운 90° 방향에 정렬한다.
- 배치 미리보기 규격은 2×1 타일이며 세로 정렬 시 1×2로 전환한다.
...
- 설치 중 카메라 줌이 변하지 않고, 종료 직후 동적 줌이 자연스럽게 재개된다.
- 커서가 같은 타일 내부에서 움직이면 미리보기 좌표가 유지되고 타일 경계를 넘을 때만 한 칸씩 이동한다.
- 방벽 미리보기와 월드 회전이 2×1/1×2 타일 방향과 일치한다.
